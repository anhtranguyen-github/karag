...
export function SettingsManager({ onClose, workspaceId, workspaceName }: { onClose?: () => void, workspaceId?: string, workspaceName?: string }) {
    ...
    const renderSettingRow = (key: keyof AppSettings, label: string, description: string) => {
        ...
        return (
            <div className={cn(
                "group relative py-6 border-b transition-all border-white/5 last:border-0",
                !mutable && "opacity-50"
            )}>
    ...
    return (
        ...
        >
            ...
            <div className="flex flex-1 min-h-0 overflow-hidden">
                {/* Nav Sidebar */}
                <aside className="w-72 border-r border-white/5 p-8 flex flex-col gap-3 shrink-0">
                    {tabs.map((tab) => (
                        <button
                            key={tab.id}
            ...
            {/* Interaction Bar */}
            <footer className="px-12 py-8 border-t border-white/5 flex items-center justify-between shrink-0">
