...
async def test_workspace_service_create(mocker):
    ...
    mocker.patch(
        "src.backend.app.core.settings_manager.settings_manager.update_settings",
        new=AsyncMock(),
    )

    ws_result = await workspace_service.create(
        {"name": "New Workspace", "description": "Description"}, user_id="test-user"
    )
...
async def test_document_service_delete(mocker):
    ...
    mock_minio = mocker.patch("src.backend.app.services.document.storage_service.minio_manager.delete_file")
    ...
    # Implementation uses content_hash for delete_many if present
    mock_col.delete_many.assert_called_once_with({"content_hash": "hash-123"})
