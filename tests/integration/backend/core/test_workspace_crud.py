...
async def test_workspace_create_basic(mocker):
    ...
    with patch("src.backend.app.core.mongodb.mongodb_manager.get_async_database", return_value=mock_db):
        ...
        mocker.patch("src.backend.app.core.settings_manager.settings_manager.get_settings", new=AsyncMock())
...
async def test_workspace_update(mocker):
    ...
    assert failures == len(WORKSPACE_NAME_FORBIDDEN)
...
async def test_workspace_delete(mocker):
    ...
    with patch("src.backend.app.core.mongodb.mongodb_manager.get_async_database", return_value=mock_db):
        ...
        with patch("src.backend.app.services.document_service.document_service", mock_doc_service):
