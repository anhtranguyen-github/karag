...
from src.backend.app.main import app as fastapi_app
...
async def test_summarization_flow(mocker):
    ...
    mocker.patch("src.backend.app.graph.nodes.get_llm", new=AsyncMock(return_value=mock_llm))
    ...
    mocker.patch("src.backend.app.graph.nodes.rag_service.search", new=AsyncMock(return_value=[]))
...
async def test_thread_api_endpoints(mocker):
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as ac:
        res = await ac.patch(
            f"/api/v1/workspaces/default/chat/threads/{thread_id}/title", json={"title": "Updated Title"}
        )
