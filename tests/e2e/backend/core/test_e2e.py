...
async def test_e2e_document_flow(async_client):
    ...
    with patch(
        "src.backend.app.api.v1.documents.document_service.upload",
...
async def test_e2e_chat_threads(async_client):
    ...
    with (
        patch(
            "src.backend.app.api.v1.chat.chat_service.list_threads",
            new=AsyncMock(return_value=[{"id": "t1", "title": "Test"}]),
        ),
        patch("src.backend.app.api.v1.chat.chat_service.update_title", new=AsyncMock()),
        patch("src.backend.app.api.v1.chat.chat_service.delete_thread", new=AsyncMock()),
    ):
