...
if not _FORCE_CLOUD:
    ...
    os.environ["NEO4J_PASSWORD"] = os.environ.get(
        "TEST_NEO4J_PASSWORD", "neo4j_password"
    )
    os.environ["QDRANT_URL"] = os.environ.get(
        "TEST_QDRANT_URL", "http://localhost:6333"
    )
    os.environ["QDRANT_API_KEY"] = os.environ.get(
        "TEST_QDRANT_API_KEY", "local-dev-key"
    )
    os.environ["MINIO_ENDPOINT"] = os.environ.get(
        "TEST_MINIO_ENDPOINT", "localhost:9000"
    )
    os.environ["MINIO_ACCESS_KEY"] = os.environ.get(
        "TEST_MINIO_ACCESS_KEY", "minioadmin"
    )
    os.environ["MINIO_SECRET_KEY"] = os.environ.get(
