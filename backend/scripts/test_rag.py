...
class RAGTestClient:
    ...
    def create_workspace(self, name: str) -> dict:
        ...
        response = self.session.post(f"{self.base_url}{API_PREFIX}/workspaces", json={"name": name})
        if response.status_code == 200:
            return response.json()
        return None
    ...
    def get_task_status(self, task_id: str) -> dict:
        ...
        response = self.session.get(f"{self.base_url}{API_PREFIX}/tasks?limit=100")
        if response.status_code == 200:
    ...
    def search_documents(self, workspace_id: str, query: str) -> dict:
        ...
        response = self.session.get(
            f"{self.base_url}{API_PREFIX}/workspaces/{workspace_id}/search", params={"q": query}
        )
        if response.status_code == 200:
            return response.json()
        return None
    ...
    def chat_completion(self, workspace_id: str, message: str, mode: str = "rag") -> dict:
        ...
        response = self.session.post(
            ...
            json={"message": message, "thread_id": str(uuid.uuid4()), "execution": {"mode": mode}},
            stream=True,
        )
        if response.status_code == 200:
            # Parse SSE response
...
def test_rag_system():
    ...
    pdf_path = os.environ.get(
        ...
        os.path.join(
            ...
            "example",
            "Building an Exam Aim Tracker.pdf",
        ),
    )
                ...
                for i in range(max_wait):
                    ...
                    if task_result and task_result.get("success"):
                        ...
                        if status == "completed":
                            ...
                            print(
                                f"  ✗ Document processing failed: {task_data.get('error', 'Unknown error')}"
                            )
                            break
                        elif i % 10 == 0:
    ...
    if docs_result and docs_result.get("data"):
        search_queries = ["exam", "tracker", "goal"]
