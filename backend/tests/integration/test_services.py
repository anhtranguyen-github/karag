...
async def test_workspace_service_create(mocker):
    ...
    mocker.patch(
        "backend.app.core.settings_manager.settings_manager.update_settings",
        new=AsyncMock(),
    )
...
async def test_document_service_delete(mocker):
    ...
    mock_qdrant_client.collection_exists = AsyncMock(
        return_value=False
    )  # Skip qdrant loops for simplicity
    mock_qdrant_client.delete = AsyncMock()
    mocker.patch(
        "backend.app.services.document.storage_service.qdrant.client",
        mock_qdrant_client,
    )
