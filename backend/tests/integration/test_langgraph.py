...
async def test_langgraph_fast_mode_direct_path():
    ...
    with patch("backend.app.rag.graph.nodes.LangChainFactory.get_llm") as mock_get_llm:
        ...
        mock_llm.ainvoke.return_value.__await__ = MagicMock(
            return_value=iter([MagicMock(content="Mock response")])
        )
...
async def test_langgraph_thinking_mode_loop():
    ...
    settings = RuntimeSettings(
        mode=ExecutionMode.THINKING, thinking=ThinkingModeConfig(max_loops=2)
    )
    initial_state = {
        "query": "Deep question",
        "workspace_id": "test-ws",
...
async def test_langgraph_max_loops_exit():
    ...
    settings = RuntimeSettings(
        mode=ExecutionMode.THINKING, thinking=ThinkingModeConfig(max_loops=1)
    )
    initial_state = {
    ...
    with patch("backend.app.rag.graph.nodes.LangChainFactory.get_llm") as mock_get_llm:
        ...
        mock_llm.ainvoke = AsyncMock(
            side_effect=[
                ...
                ),  # reflect_and_decide loop 0 -> increments loop_count to 1
                MagicMock(content="refined query"),  # build_query_context loop 1
        ...
        with patch("backend.app.rag.rag_service.rag_service.search") as mock_search:
            ...
            assert final_state["loop_count"] == 1
            assert (
                final_state["is_sufficient"] is True
            )  # Forced True by max_loops logic for exit
