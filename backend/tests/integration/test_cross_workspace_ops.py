...
async def test_link_document_starts_as_indexing(mocker):
    ...
    mocker.patch("backend.app.core.settings_manager.settings_manager.get_settings", new=AsyncMock(return_value=MagicMock()))
    mock_ingestion = mocker.patch("backend.app.services.document.document_ingestion_service.document_ingestion_service.index_document", new=AsyncMock(return_value=5))
