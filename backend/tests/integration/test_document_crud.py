...
async def test_cross_workspace_share_document():
    ...
    with patch('backend.app.core.mongodb.mongodb_manager.get_async_database', return_value=mock_db):
        with patch('backend.app.services.task.task_service.task_service.is_cancelled', new=AsyncMock(return_value=False)):
            with patch('backend.app.services.task.task_service.task_service.update_task', new=AsyncMock()):
                ...
                with patch('backend.app.services.document.document_ingestion_service.document_ingestion_service.index_document', new=AsyncMock()):
