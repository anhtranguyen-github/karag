...
async def test_link_performs_full_index(mocker, mock_db_and_col):
    ...
    mock_index = mocker.patch("backend.app.services.document.document_ingestion_service.DocumentIngestionService.index_document", return_value=10)
...
async def test_update_settings_persists_to_db(mocker, mock_db_and_col):
    ...
    mocker.patch.object(settings_manager, "get_settings_metadata", return_value={"chunking.chunk_size": {"mutable": True}})
...
async def test_ingestion_progress_updates(mocker, mock_db_and_col):
    ...
    doc = {
        "id": "doc_1", "filename": "test.txt", "extension": ".txt", 
        "minio_path": "path/test.txt", "content_hash": "h1"
    }
    ...
    mocker.patch("backend.app.rag.ingestion.ingestion_pipeline.get_target_collection", return_value=("knowledge_base_1536", 1536))
    ...
    mocker.patch("backend.app.rag.rag_service.rag_service.get_embeddings", return_value=[[0.1]*1536])
    mocker.patch("backend.app.rag.qdrant_provider.qdrant.upsert_documents")
    mocker.patch("backend.app.rag.qdrant_provider.qdrant.client.collection_exists", return_value=True)
