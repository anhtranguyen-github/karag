...
async def get_isolation_context(
    request: Request, api_key: Annotated[str, Depends(get_api_key_from_request)]
) -> IsolationContext:
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail={"success": False, "code": "INVALID_API_KEY", "message": str(e)},
        )


async def require_permission(permission: str, context: IsolationContext) -> None:
    """
...
async def get_current_workspace_id(context: IsolationContextDep) -> str:
    """
...
async def require_read_permission(context: IsolationContextDep) -> IsolationContext:
    """Dependency that requires 'read' permission."""
    await require_permission("read", context)
    return context


async def require_write_permission(context: IsolationContextDep) -> IsolationContext:
    """Dependency that requires 'write' permission."""
    await require_permission("write", context)
    return context


async def require_delete_permission(context: IsolationContextDep) -> IsolationContext:
    """Dependency that requires 'delete' permission."""
    await require_permission("delete", context)
    return context


async def require_admin_permission(context: IsolationContextDep) -> IsolationContext:
