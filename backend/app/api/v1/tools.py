...
async def add_tool(tool: ToolDefinition):
    ...
    if tool_manager.get_tool_definition(tool.id):
        return AppResponse.business_failure(
            code="DUPLICATE_TOOL", message=f"Tool with ID '{tool.id}' already exists"
        )
    result = tool_manager.add_tool(tool)
    return AppResponse.success_response(
        data=result, message=f"Tool '{tool.id}' registered"
    )
...
async def toggle_tool(tool_id: str, enabled: bool):
    ...
    return AppResponse.success_response(
        data=tool, message=f"Tool '{tool_id}' {'enabled' if enabled else 'disabled'}"
    )
...
async def delete_tool(tool_id: str):
    ...
    if tool.type == "system":
        return AppResponse.business_failure(
            code="PROTECTED_TOOL", message="Cannot delete system protected tools"
        )

    tool_manager.delete_tool(tool_id)
    return AppResponse.success_response(
        data={"id": tool_id}, code="TOOL_DELETED", message=f"Tool {tool_id} deleted"
    )
