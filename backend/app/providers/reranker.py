    ...
    async def rerank(self, query: str, documents: List[Dict[str, Any]], top_k: int) -> List[Dict[str, Any]]:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code != 200:
                logger.error("cohere_rerank_failed", status=response.status_code, text=response.text)
...
class JinaReranker(RerankerProvider):
    ...
    async def rerank(self, query: str, documents: List[Dict[str, Any]], top_k: int) -> List[Dict[str, Any]]:
        ...
        async with httpx.AsyncClient() as client:
            response = await client.post(url, headers=headers, json=data)
            if response.status_code != 200:
                logger.error("jina_rerank_failed", status=response.status_code, text=response.text)
                return documents[:top_k]
