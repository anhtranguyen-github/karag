...
```bash
python backend/scripts/eval_download.py --list
```
...
python backend/scripts/eval_download.py --dataset ms_marco

# Download a group of datasets
python backend/scripts/eval_download.py --dataset-group english_qa

# Download all datasets
python backend/scripts/eval_download.py --all
```
...
python backend/scripts/eval_run.py --benchmark smoke_test

# List available benchmarks
python backend/scripts/eval_run.py --list-benchmarks

# Run with custom settings
python backend/scripts/eval_run.py --dataset ms_marco --runner standard_qa --max-samples 100 --output results.json
```
...
python backend/scripts/eval_report.py --input results.json --format markdown

# Compare two runs
python backend/scripts/eval_report.py --compare baseline.json new.json --output comparison.md
```
...
from backend.app.eval.datasets import dataset_registry
...
from backend.app.eval.runners import StandardQARunner, RunnerConfig
...
```python
from backend.app.eval.metrics import RetrievalMetrics, GenerationMetrics
...
Edit `backend/app/eval/config/datasets.yaml` to add new datasets:
...
Edit `backend/app/eval/config/benchmarks.yaml` to define benchmarks:
...
```python
from backend.app.eval.datasets import BaseDatasetLoader, DatasetEntry, DatasetSplit
...
```python
from backend.app.eval.runners import BaseRunner, RunnerConfig, RunnerResult
...
from backend.app.eval import StandardQARunner, RunnerConfig
