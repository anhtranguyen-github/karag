...
class EvalFramework:
    ...
    def _aggregate_results(self, results: List[Dict]) -> Dict[str, Any]:
        ...
        return {
            ...
            "overall_score": sum(
                s["mean"] for s in summary.values()
...
def create_rag_evaluator(llm_client) -> EvalFramework:
    ...
    framework.add_evaluator(
        ...
        LLMBasedEvaluator(llm_client, EvalMetric.RELEVANCE)
    )
    framework.add_evaluator(
        EvalMetric.GROUNDEDNESS,
        LLMBasedEvaluator(llm_client, EvalMetric.GROUNDEDNESS)
    )
    framework.add_evaluator(
        EvalMetric.COHERENCE,
        LLMBasedEvaluator(llm_client, EvalMetric.COHERENCE)
    )
    framework.add_evaluator(
        EvalMetric.HELPFULNESS,
        LLMBasedEvaluator(llm_client, EvalMetric.HELPFULNESS)
    )
    ...
    framework.add_evaluator(
        EvalMetric.SAFETY,
        RuleBasedEvaluator(EvalMetric.SAFETY, [safety_check])
    )
