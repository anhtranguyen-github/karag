...
class PromptRegistry:
    ...
    async def record_outcome(
        ...
        metrics["avg_latency_ms"] = (
            (metrics["avg_latency_ms"] * uses + new_latency) / (uses + 1)
        )
        
        # Success rate
        success = outcome.get("success", True)
        metrics["success_rate"] = (
            (metrics["success_rate"] * uses + (1.0 if success else 0.0)) / (uses + 1)
        )
        ...
        if "user_rating" in outcome:
            if "avg_rating" not in metrics:
                ...
                metrics["avg_rating"] = (
                    (metrics["avg_rating"] * uses + outcome["user_rating"]) / (uses + 1)
                )
