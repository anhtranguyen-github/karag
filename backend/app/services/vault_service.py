...
class VaultService:
    ...
    async def initialize_global_vault(cls) -> Vault:
        ...
        global_vault = Vault(
            ...
            vector_store_config=VectorStoreConfig(
                collection_name="global_kb", dimension=1536, distance_metric="cosine"
            ),
    ...
    async def create_workspace_vault(
        ...
        existing = await db.vaults.find_one(
            {"owner_workspace_id": workspace_id, "name": name}
        )
        if existing:
            raise ConflictError(
                f"Vault with name '{name}' already exists in this workspace"
            )
        ...
        vault = Vault(
            ...
            file_store_config=FileStoreConfig(
                bucket="rag-docs", prefix=f"workspaces/{workspace_id}/"
            ),
        )
        ...
        await db.workspaces.update_one(
            {"id": workspace_id}, {"$push": {"vault_ids": vault_id}}
        )

        logger.info(
            "vault_created", vault_id=vault_id, workspace_id=workspace_id, name=name
        )
    ...
    async def get_vault(
        ...
        if vault.type == "global":
            ...
            if vault.is_read_only and required_permission != "read":
                raise AuthorizationError(
                    f"Vault '{vault_id}' is read-only", details={"vault_id": vault_id}
                )
    ...
    async def list_workspace_vaults(
        cls, workspace_id: str, include_global: bool = True
    ) -> List[Vault]:
        """
        ...
        cursor = db.vaults.find(
            {"type": "workspace", "owner_workspace_id": workspace_id, "is_active": True}
        )
    ...
    async def get_or_create_default_vault(cls, workspace_id: str) -> Vault:
        ...
        vault = await db.vaults.find_one(
            {"owner_workspace_id": workspace_id, "name": cls.DEFAULT_WORKSPACE_VAULT}
        )
    ...
    async def delete_vault(
        cls, vault_id: str, workspace_id: str, delete_contents: bool = False
    ) -> bool:
        """
        ...
        # Update workspace
        await db.workspaces.update_one(
            {"id": workspace_id}, {"$pull": {"vault_ids": vault_id}}
        )
    ...
    def get_collection_name(
        cls, workspace_id: str, vault_id: Optional[str] = None
    ) -> str:
        """
