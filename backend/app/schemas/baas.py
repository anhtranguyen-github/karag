...
class Workspace(BaseModel):
    ...
    owner_id: Optional[str] = Field(None, description="Reference to owner user (for admin)")
    
    # Isolation configuration
    vault_ids: List[str] = Field(default_factory=list, description="Associated vault IDs")
...
class APIKey(BaseModel):
    ...
    key_prefix: str = Field(..., min_length=8, max_length=16, 
...
class Vault(BaseModel):
    ...
    owner_workspace_id: Optional[str] = Field(
        None, 
        description="None for global vaults, workspace_id for workspace vaults"
    )
...
class Chunk(BaseModel):
    ...
    metadata: Dict[str, Any] = Field(default_factory=dict, 
                                      description="Source, headers, etc.")
