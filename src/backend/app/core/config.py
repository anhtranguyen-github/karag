...
def _load_environment() -> None:
    ...
    candidates = (
        current.parents[2] / ".env", # Backend local (high priority)
        current.parents[4] / ".env", # Root fallback (low priority)
    )
...
class PlatformSettings:
    ...
    default_reranker_model: str = os.getenv("DEFAULT_RERANKER_MODEL", "jina-reranker-v3")
    ...
    rag_default_chunker: str = os.getenv("RAG_DEFAULT_CHUNKER", "semantic")
    rag_default_embedder: str = os.getenv("RAG_DEFAULT_EMBEDDER", "multi_vector")
    rag_default_vectorstore: str = os.getenv("RAG_DEFAULT_VECTORSTORE", "qdrant")
    rag_default_reranker: str = os.getenv("RAG_DEFAULT_RERANKER", "colbert")
    rag_default_query_transformer: str = os.getenv("RAG_DEFAULT_QUERY_TRANSFORMER", "hyde")
    rag_default_generator: str = os.getenv("RAG_DEFAULT_GENERATOR", "openai")
    rag_default_retriever: str = os.getenv("RAG_DEFAULT_RETRIEVER", "multi_stage")

    default_qdrant_collection: str = os.getenv("DEFAULT_QDRANT_COLLECTION", "cascade_collection")
    ...
    # print(f"DEBUG: MINIO_ENDPOINT={os.getenv('MINIO_ENDPOINT')}")
    ...
    @property
    def embedding_component(self) -> str:
        return self.rag_default_embedder

    @property
    def generator_component(self) -> str:
        return self.rag_default_generator

    @property
    def reranker_component(self) -> str:
        return self.rag_default_reranker

    @property
    def vectorstore_component(self) -> str:
        return self.rag_default_vectorstore
