...
async def test_link_performs_full_index(mocker, mock_db_and_col):
    ...
    mock_index = mocker.patch(
        "src.backend.app.services.document.document_ingestion_service.DocumentIngestionService.index_document",
        return_value=10,
    )

    from src.backend.app.services.document.document_ingestion_service import (
...
async def test_ingestion_progress_updates(mocker, mock_db_and_col):
    ...
    mocker.patch("src.backend.app.core.minio.minio_manager.get_file", return_value=b"some")
    mocker.patch("src.backend.app.core.path_utils.validate_safe_path", side_effect=lambda x: Path(x))
    mocker.patch("src.backend.app.core.path_utils.get_safe_temp_path", return_value=Path("temp.txt"))
    mocker.patch("os.remove")
    mocker.patch("os.path.exists", return_value=True)
    mocker.patch("builtins.open", mocker.mock_open())

    mocker.patch("src.backend.app.rag.ingestion.ingestion_pipeline.initialize", new_callable=AsyncMock)
    mocker.patch(
        "src.backend.app.rag.ingestion.ingestion_pipeline.get_ingestion_config",
    ...
    mocker.patch("src.backend.app.rag.ingestion.TextLoader", return_value=mock_loader)
    mocker.patch("src.backend.app.rag.rag_service.rag_service.chunk_text", new_callable=AsyncMock, return_value=["chunk1"])
    mocker.patch(
        "src.backend.app.rag.rag_service.rag_service.get_embeddings",
        new_callable=AsyncMock,
    ...
    mocker.patch(
        "src.backend.app.core.factory.ProviderFactory.get_vector_store",
    ...
    mocker.patch("src.backend.app.services.task.task_service.task_service", mock_task)
    mocker.patch("src.backend.app.services.document.document_ingestion_service.task_service", mock_task)
