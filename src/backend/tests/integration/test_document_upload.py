...
async def test_upload_to_vault_auto_indexes(mocker):
    ...
    # Mocking MinIO
    mocker.patch("src.backend.app.core.minio.MinioManager.upload_file", new=AsyncMock())
...
async def test_upload_resilience_on_qdrant_403(mocker):
    ...
    mocker.patch(
        "src.backend.app.rag.store.qdrant.QdrantStore.create_collection_if_not_exists",
