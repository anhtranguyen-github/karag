...
async def test_rag_get_embeddings():
    ...
    with patch("src.backend.app.core.factory.ProviderFactory.get_embeddings") as mock_get:
...
async def test_rag_search_basic():
    ...
    with patch("src.backend.app.core.factory.ProviderFactory.get_vector_store") as mock_store:
        with patch("src.backend.app.core.factory.ProviderFactory.get_embeddings") as mock_embed:
            # Setup mocks
            mock_provider = AsyncMock()
            mock_provider.embed_query = AsyncMock(return_value=[0.1] * 1536)
            ...
            with patch(
                "src.backend.app.core.settings_manager.settings_manager.get_settings",
...
async def test_rag_search_with_reranking():
    ...
    with patch("src.backend.app.core.factory.ProviderFactory.get_vector_store") as mock_store:
        with patch("src.backend.app.core.factory.ProviderFactory.get_embeddings") as mock_embed:
            with patch("src.backend.app.providers.reranker.get_reranker") as mock_reranker:
                # Setup mocks
                mock_provider = AsyncMock()
                mock_provider.embed_query = AsyncMock(return_value=[0.1] * 1536)
                mock_embed.return_value = mock_provider
                ...
                with patch(
                    ...
                    await rag_service.search(query="test query", workspace_id="test_ws", limit=5)
...
async def test_rag_execute_generation():
    ...
    from src.backend.app.rag.graph.orchestrator import rag_executor
...
async def test_rag_contextual_compression():
    ...
    from src.backend.app.rag.advanced_retrieval import ContextualCompressor
...
async def test_rag_multi_query_retriever():
    ...
    from src.backend.app.rag.advanced_retrieval import MultiQueryRetriever
